...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, n_times_crossframe_attn_in_self=0, return_attn=False, attn_cache=None):
        ...
        if return_attn and attn_cache is not None:
            next_idx = int(getattr(attn_cache, "_obi_next_idx", 0))
            cache_item = {
                "attn": attn,
                "query_len": int(attn.shape[1]),
                "key_len": int(attn.shape[2]),
                "heads": int(h),
                "source": "attn2_temporal_cross",
                "temporal_attn_index": next_idx,
            }
            attn_cache.append(cache_item)
            try:
                attn_cache._obi_next_idx = next_idx + 1
...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x, context_temp=None, return_attn=False, attn_cache=None):
        ...
        num_frames = getattr(self, "num_frames", None)
        if num_frames is None:
            num_frames = getattr(self, "sequence_length", None)
        if num_frames is None:
            raise ValueError(
                "TemporalTransformer.forward requires self.num_frames or self.sequence_length"
            )
        num_frames = int(num_frames)
        if bf % num_frames != 0:
            raise ValueError(
                f"Invalid shape for temporal reshape: bf={bf}, num_frames={num_frames}"
            )

        if not self.use_linear:
            x = rearrange(x, "(b f) c h w -> (b h w) c f", f=num_frames).contiguous()
            x = self.proj_in(x)
            x = rearrange(x, "(bhw) c f -> (bhw) f c")
        if self.use_linear:
            x = rearrange(x, "(b f) c h w -> b (h w) f c", f=num_frames).contiguous()
            x = self.proj_in(x)

        context_for_blocks = context_temp
        for i, block in enumerate(self.transformer_blocks):
            if i == 0:
                context_for_blocks = repeat(
                    context_temp, "b m d -> (b r) m d", r=(h * w) // num_frames
                )
            x = block(
                x,
                context=context_for_blocks,
