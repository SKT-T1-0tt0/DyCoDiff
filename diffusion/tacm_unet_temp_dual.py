...
class UNetModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 显式形参与 **kwargs 任一为 True 即开启（并 pop 掉以免向下游重复传递）
        return_attn = bool(return_attn) or bool(kwargs.pop("return_attn", False))
        # OBI / 调试：可传入同一 list 引用，避免 DDP 与二次前向时仅依赖 self 上挂 cache 失效
        explicit_attn_cache = kwargs.pop("attn_cache", None)
        if return_attn:
            if explicit_attn_cache is not None:
                self._temp_attn_cache = explicit_attn_cache
                self._attn_cache = explicit_attn_cache
                attn_cache = explicit_attn_cache
            else:
                self._temp_attn_cache = []
                self._attn_cache = self._temp_attn_cache
                attn_cache = self._temp_attn_cache
        else:
            attn_cache = None
            self._temp_attn_cache = None
