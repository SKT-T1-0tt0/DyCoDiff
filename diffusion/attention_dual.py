...
class DualTemporalTransformer(nn.Module):
    ...
    def forward(
        ...
        for i in range(2):
            condition_state = encoder_hidden_states[:, tokens_start : tokens_start + self.condition_lengths[i]]
            transformer_index = self.transformer_index_for_condition[i]
            branch_return_attn = return_attn
            branch_attn_cache = attn_cache
            out = self.transformers[transformer_index](
                input_states,
                context_temp=condition_state,
                return_attn=branch_return_attn,
                attn_cache=branch_attn_cache,
            )
            encoded_state = out[0] if isinstance(out, (tuple, list)) else out
            encoded_states.append(encoded_state - input_states)
